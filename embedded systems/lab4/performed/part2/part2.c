...
static ssize_t device_write(struct file *filp, char *buffer, size_t length, loff_t *offset)
{
    ...
    if (copy_from_user (to_load, buffer, 8) != 0)
    {
		printk (KERN_ERR "Error: copy_from_user unsuccessful");
        return -1;
    }

    if (to_load[2] != ':' | to_load[5] != ':')
    {
		printk ("Error: wrong format, correct format: MM:SS:DD");
        return -1;
    }  

    sscanf(&(to_load[0]), "%2d", &MM);
    sscanf(&(to_load[3]), "%2d", &SS);
    sscanf(&(to_load[6]), "%d", &DD);

    return strlen(to_load);
}
